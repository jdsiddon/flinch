...
var data = {
	...
	convert: function(file, callback) {
		...
		fs.readFile(filePath, 'utf8', function (err, data) {
			...
			//console.log("as Reading File: " + data);
	...
	/* Write data array to text file in 'tests'
	...
	parse: function(data, cb) {
		...
		for (var i = 0; i < formattedData.length; i++) {
			// Add newline to end of each element (each data point is still a string).
		//	formattedData[i] = formattedData[i].concat('\n');
			// Split the string into an array by commas making formattedData an array of arrays.
		//	tempArray = formattedData[i].split(",");
			console.log(formattedData[i]);
			// Reassign as an array.
		//	formattedData[i] = tempArray;
			//console.log("before slice = " + formattedData[i]);
			//formattedData[i] = formattedData[i].slice(1);
			//console.log("After slice = " + formattedData[i]);
		}
		// Remove last element from formattedData as it holds a \r\n which we dont want.
		formattedData.pop();
		//console.log('formattedData BEFORE LOOP: ' + formattedData);
		cb(formattedData);
	}
